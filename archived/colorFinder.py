...
print("Copy and paste the below into config.toml under [fancy-colors]:")
for c in result:
    print(f"\"{unescape(c[1].encode('utf-8').decode('unicode_escape').encode('latin1').decode('utf-8').title())}\" = {hexStrToRGB(c[0])}")
