...
class ResNetModelClassifier(nn.Module):
    def __init__(self, _num_classes: int):
        super().__init__()
        # Using ResNet50 as the "best" balance of performance and depth
        self.base = models.resnet50(weights=models.ResNet50_Weights.DEFAULT)

        # ResNet-50's final layer is 'fc'. We replace it with your custom head.
        in_features = self.base.fc.in_features

        # Stripping the original fc and replacing with a specialized classifier
...
#  Data transforms
# ────────────────────────────────────────────────
# I used the 'ApplyTransform' wrapper logic here to ensure validation stays clean
...
train_transform_pattern = transforms.Compose([
    ...
    # transforms.ColorJitter(contrast=0.4, hue=0.1, saturation=0.25),
    transforms.RandomRotation(degrees=15),
    transforms.Resize(pattern_full_size, interpolation=transforms.InterpolationMode.BILINEAR),
...
def train(model_name: str, training_dir: str, batch_size: int, plot=False):
    ...
    model = ResNetModelClassifier(num_classes).to(device)
...
if __name__ == '__main__':
    ...
    train(f"resnet/wzeave_best_model{"_grayscale" if pattern_grayscale else ""}", weave_training_dir, weave_batch_size, True)
    train(f"resnet/pattern_best_model{"_grayscale" if pattern_grayscale else ""}", pattern_training_dir, pattern_batch_size, True)
